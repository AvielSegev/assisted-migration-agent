...
func newSyncTestDB(pool *store.Pool, tmpDir, name string) (*store.Database, *store.Store2) {
...
func insertSyncTestVM(ctx context.Context, st *store.Store2, vmID, name string) {
	...
	Describe("syncAttached", func() {
		Context("groups", func() {
			It("copies group definitions preserving ID and created_at, setting updated_at to now", func() {
				...
				Expect(v2.SyncAttached(ctx, prevSt, attachedSchema, syncTime)).To(Succeed())
				detach()
			...
			It("is a no-op when the previous collection has no groups", func() {
				detach := attachNew()
				defer detach()
				Expect(v2.SyncAttached(ctx, prevSt, attachedSchema, time.Now())).To(Succeed())
				detach()
		...
		Context("VM labels", func() {
			It("copies user labels to matching VMs, excluding system labels", func() {
				...
				Expect(prevSt.VM().AddLabel(ctx, "vm-1", v2.LabelNew)).To(Succeed())
				...
				Expect(labelsMap["vm-1"]).NotTo(ContainElement(v2.LabelNew))
			})
		...
		Context("new VM labeling", func() {
			It("adds LabelNew to VMs absent from the previous collection", func() {
				...
				Expect(labelsMap["vm-brand-new"]).To(ContainElement(v2.LabelNew))
			})
		})
	})
