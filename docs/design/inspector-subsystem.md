...
TERMINAL STATES: ready, done, error, canceled (IsBusy() = false)
```
...
The Work Builder Layer is a factory pattern implementation that constructs executable work units for the inspector workflow. It serves as an abstraction layer between the service orchestration logic and the actual VMware vSphere operations.

#### Purpose and Design Rationale

**What it does:**
- **Decouples workflow definition from execution**: The service layer doesn't need to know the specific details of how to connect to vSphere or perform VM operations. It only needs to request work units and execute them.
- **Encapsulates work unit creation**: All the logic for creating vSphere connections, building snapshot operations, and composing inspection workflows is centralized in the builder.
- **Enables testability**: The builder can be easily mocked or replaced with test implementations, allowing the service layer to be tested independently of VMware operations.
- **Provides flexibility**: Different work builders can implement the same interface, enabling different inspection strategies (e.g., different snapshot approaches, different inspection depths) without changing the service code.

**Why we use it:**
1. **Separation of Concerns**: The `InspectorService` focuses on state management, queue processing, and coordination, while the work builder focuses on defining what work needs to be done.
2. **Reusability**: The same work builder pattern is used in the Collector service (`pkg/collector/v1/work_builder.go`), providing consistency across the codebase.
3. **Testability**: Mock work builders can simulate various scenarios (connection failures, VM errors, etc.) without requiring actual vSphere infrastructure.
4. **Maintainability**: Changes to the inspection workflow (e.g., adding new steps, modifying snapshot behavior) are isolated to the work builder, reducing the risk of breaking service logic.

**How it fits into the architecture:**
The Work Builder sits between the Service Layer and the vSphere API. When the `InspectorService` needs to process a VM:
1. It calls `builder.Build(moid)` to get a sequence of work units for that VM
2. Each work unit is a self-contained function that can be executed by the scheduler
3. The scheduler executes the work units sequentially, handling context cancellation and errors
4. The service updates state based on work unit results

This pattern allows the service to remain agnostic about the specific operations being performed, making it easier to extend or modify the inspection workflow in the future.
